...
# ==============================
def plot_curve():
    df = pd.read_csv("./log.lammps",skiprows=54,sep=r"\s+",nrows=5000,usecols=[0,1,2,3,4])
    ...
    plt.plot(time, pe, color = "r", label = 'Potential Energy')
    plt.plot(time, etot, color = "b", label = 'Total Energy')
    plt.xlabel(r"$Time~[ps]$")
    plt.ylabel(r"$E$ (eV)")
    plt.legend()
    plt.tight_layout()
    plt.savefig(f"EnergyNPT_MoSe2.{save_format}", dpi=600)
    plt.close()
